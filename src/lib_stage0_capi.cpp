// lib_stage0_capi.cpp — C API shared library wrapping the stage-0
// multi-core pipeline. Same internal architecture as s_stage0_multicore.cpp:
// N shard threads, per-shard SPSC ring, halo=2 routing. Exposed for ctypes
// from Python (live DVXplorer driver in deploy/orin/live_stage0_bench.py).
...
struct ShardCtx {
    ...
    std::atomic<std::uint64_t> pass_owner{0};
};
...
void shard_worker(ShardCtx* ctx) {
    ...
    while (true) {
        ...
        if (m.is_owner) {
            ...
            if (pass) ctx->pass_owner.fetch_add(1, std::memory_order_relaxed);
        } else {
...
//   [2]  pass_owner  (events surviving stage-0 tdrop)
...
void s0_snapshot_stats(S0Handle* h, double* out) {
    ...
    std::uint64_t totalo = 0, totalh = 0, passo = 0;
    ...
    for (auto& c : h->ctxs) {
        ...
        passo  += c->pass_owner.load(std::memory_order_relaxed);
    ...
    out[2] = static_cast<double>(passo);
...
void s0_reset_stats(S0Handle* h) {
    ...
    for (auto& c : h->ctxs) {
        ...
        c->pass_owner.store(0, std::memory_order_relaxed);
