"""Live DVXplorer + stage-0 multi-core benchmark.

Architecture:
  Python main thread:
    - opens dv_processing camera (DVXplorer Micro on aarch64; libcaer
      can't open the Micro variant, so we must use dv_processing).
    - tight non-blocking loop reading event batches from camera (no
      polling sleep — minimum-latency path).
    - applies a token-bucket rate cap (default 1 MHz steady, 100k bucket
      = ~100 ms burst) so we never overrun the C++ stage-0 pipeline.
    - packs accepted events into a (n, 4) float32 ndarray [t_us, x, y, p]
      and ctypes-calls libstage0_capi.so::s0_submit_batch.
    - prints throughput + per-event latency every 1 s.
    - runs for at least 30 s (configurable via --duration).

  C++ background threads (libstage0_capi.so):
    - N shard threads, each pinned to its own core.
    - per-shard SPSC ring receives events; halo=2 routing.
    - per-event latency measured TSC-precise from submit to owner-done.

Usage:
    python3 deploy/orin/live_stage0_bench.py
        --weights /tmp/ssla_s_random
        --shards 4 --halo 2 --base-core 2
        --rate-mev 1.0 --bucket-events 100000
        --duration 30
"""
...
LIB_PATH = os.path.abspath(os.path.join(
    os.path.dirname(__file__), "..", "build", "libstage0_capi.so"))


def load_lib():
    if not os.path.exists(LIB_PATH):
        sys.exit(f"[fatal] {LIB_PATH} not found — build first:\n"
                 f"    cmake --build deploy/build --target stage0_capi -j6")
...
def main():
    ...
    print(f"    stage-0 passed : {int(stats_arr[2]):>10}  "
          f"({100.0*stats_arr[2]/max(1,stats_arr[0]):.2f}%)")
