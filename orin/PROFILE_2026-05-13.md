# SSLA-S Full-Pipeline Profile (2026-05-13)
...
|---:|---:|---:|---|
| 0.5 | **65.9** | 0 | well below saturation, no backlog |
| 1.0 | **131.9** | 0 | well below saturation |
| 1.5 | **197.9** | 0–3 | just below saturation |
| 2.0 | **224.8** | **22–28k** | **saturated** |
| 2.5 | 222.0 | 21–27k | saturated |
| 3.0 | 220.8 | 22–28k | saturated |
| 4.0 | 220.2 | 24–30k | saturated, slight regression |

**Saturation throughput ≈ 220–225 kev/s** (GPU drain, all 8 blocks combined).
...
- Mild regression above 3 Mev/s synth (220 vs 225) is most likely L2 / DRAM
  contention with the higher push pressure.
- Drain is split roughly evenly across 8 blocks now (with balanced topology):
  corner blocks ~28 kev/s, middle blocks ~27 kev/s. Pre-balancing, corners
  ran at 27 kev/s and middles plateaued at 25.9 kev/s.

## 2. End-to-end latency at saturation (synth 2.0 Mev/s)

Per-block, OWNER events only (n_owner ≈ 311–561 over 6 s window). Latency
quoted in **µs**:

```
blk | n_own |  emit→push (CPU)   kernel (GPU)        e2e (sum)
    |       |  p50 / p99 / max   p50 / p99 / max     p50 / p99 / max
----|-------|-------------------|-------------------|-------------------
  0 |  525  |   22 /  78 / 131  | 300 / 1507 / 2098 | 322 / 1585 / 2229
  1 |  329  |   22 /  73 / 105  | 321 / 1798 / 1835 | 343 / 1871 / 1940
  2 |  331  |   22 /  69 /  97  | 308 / 1505 / 2067 | 330 / 1574 / 2164
  3 |  550  |   22 /  67 / 125  | 300 / 1747 / 2187 | 322 / 1814 / 2312
  4 |  534  |   18 /  62 /  93  | 297 / 1917 / 2151 | 315 / 1979 / 2244
  5 |  345  |   17 /  57 / 105  | 307 / 1408 / 1758 | 324 / 1465 / 1863
  6 |  311  |   19 /  71 /  81  | 315 / 1905 / 2135 | 334 / 1976 / 2216
  7 |  561  |   18 /  69 / 110  | 296 / 1074 / 2136 | 314 / 1143 / 2246
```

### Summary across all blocks (saturated, synth 2.0 Mev/s)

| metric | p50 | p99 | max |
|--------|----:|----:|----:|
| **emit → ring push (CPU pipeline)** | ~20 µs | ~70 µs | ~130 µs |
| **GPU kernel** | ~305 µs | ~1700 µs | ~2150 µs |
| **whole pipeline (emit → GPU done)** | **~325 µs** | **~1800 µs** | **~2250 µs** |

### Latency vs offered load

```
synth 0.5 Mev/s — kernel p50:  131 µs        — emit→push p50:   8 µs   — e2e p50: ~140 µs
synth 1.0 Mev/s — kernel p50:  135 µs        — emit→push p50:   8 µs   — e2e p50: ~143 µs
synth 1.5 Mev/s — kernel p50:  225 µs (mix)  — emit→push p50:   8 µs   — e2e p50: ~233 µs
synth 2.0 Mev/s — kernel p50:  305 µs        — emit→push p50:  20 µs   — e2e p50: ~325 µs
synth 2.5 Mev/s — kernel p50:  305 µs        — emit→push p50:  20 µs   — e2e p50: ~325 µs
synth 3.0 Mev/s — kernel p50:  308 µs        — emit→push p50:  20 µs   — e2e p50: ~328 µs
synth 4.0 Mev/s — kernel p50:  306 µs        — emit→push p50:  20 µs   — e2e p50: ~326 µs
```
...
- **Live drain ceiling:** ~225 kev/s (1 s peak), ~220 kev/s steady state.
- **Per-block ceiling:** ~28 kev/s. With 8 blocks, total ceiling ≈ 224 kev/s.
- **CPU side:** stable up to ~2.8 Mev/s admit (post commit d876030 CPU A1),
  ...
  replication and 2× tdrops. So CPU produces 10–25% more than GPU drains.
...
- **End-to-end latency at saturation:** p50 ~325 µs, p99 ~1.8 ms,
  max ~2.3 ms. The tail (p99/p50 ≈ 5–6×) is the worst aspect.
